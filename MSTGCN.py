...
class cheb_conv(nn.Module):
    ...
    def forward(self, x):
        ...
        edge_idx, edge_attr = dense_to_sparse(adj_for_run)  # dense_to_sparse:将密集邻接矩阵转换为定义的稀疏邻接矩阵
        # edge_idx_l, edge_attr_l = get_laplacian(edge_idx, edge_attr, 'sym')
        edge_idx_l, edge_attr_l = get_laplacian(edge_idx, edge_attr)

        L_tilde = to_dense_adj(edge_idx_l, edge_attr=edge_attr_l)[0]    # 将边缘索引和边缘属性给出的批处理稀疏邻接矩阵转换为单个密集批处理邻接矩阵。
...
class MSTGCN_submodule(nn.Module):
    ...
    # def __init__(self, DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, num_for_predict, len_input):
        ...
        self.initial_conv = MSTGCN_block(in_channels,K,in_channels,nb_time_filter,1,fusiongraph,device)  # 用于将增强后的输入转换成原输入相同的格式，方便后面卷积
